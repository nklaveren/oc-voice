...
Hoje existem três lugares que comparam texto falado contra listas fixas, cada um com regra própria, e todos por igualdade exata: os comandos `classify` (`src/commands/mod.rs:155`), a tabela de aliases `resolve_target_alias` (removida em M2.1) e o filtro de alucinação `filter_hallucination` (`src/asr/mod.rs:263`). Igualdade exata é frágil contra ASR — foi o que causou o bug do "câmbio".
...
1. **Normalizar** — minúsculas, `fold_diacritics` (já existe em `src/commands/mod.rs:224`), remoção de pontuação. Colapsar `qu`→`k` e `c`→`k` na mesma passada: é uma linha e cobre a confusão acústica mais comum do português.
...
Reescrever `classify` (`src/commands/mod.rs:155`) usando o matcher, remover a guarda `words.len() > 5`, e passar o filtro de alucinação pelo mesmo caminho.
...
**O fbank está feito (`src/fbank.rs`), e a referência é o que o torna confiável.** 80 bandas Kaldi em Rust puro, medido contra `torchaudio.compliance.kaldi.fbank` sobre um chirp de banda larga: pior desvio abaixo de 0.01 no valor log-mel. O teste foi validado quebrando cada convenção e lendo o desvio — janela Hann no lugar da povey 2.94, pré-ênfase 0.97→0.95 1.48, corte de 20 Hz→0 3.28, curva mel 700→690 0.81. Duas descobertas que só apareceram por medir: o sinal original de duas senoides deixava a maioria dos 80 filtros olhando para o silêncio e não testava o banco de filtros; e a constante 1127 de Kaldi **cancela** — ela multiplica todo valor mel e as bordas dos filtros saem da mesma função, então a escala some de toda comparação. Quem molda a curva é o 700.
